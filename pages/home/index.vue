    ...
    >
      ...
      <div v-if="movie.movies.length >= 8" class="movies-category">
        <section
          ...
          :id="'section' + id + '-' + index"
        >
          <a :href="'#section' + id + '-' + (index - 1 <= 0 ? noSection : index - 1)"><</a>
          <div v-for="subIndex in noItems" :key="subIndex" class="item"
           @click="()=>{
             banner.image = $store.state.BASE_URL + movies[id].movies[( ((index - 1) * noItems ) + (subIndex - 1))].banner;
             banner.logo = $store.state.BASE_URL + movies[id].movies[( ((index - 1) * noItems ) + (subIndex - 1))].logo ;
             banner.description = movies[id].movies[( ((index - 1) * noItems ) + (subIndex - 1))].descricao;
            }">
              <img class="fadeClass" 
                v-if="movies[id] !== undefined"           
                :src="$store.state.BASE_URL + movies[id].movies[( ((index - 1) * noItems ) + (subIndex - 1))].foto"
              />                       
          </div>

          <a :href="'#section'+ id + '-' + (index + 1 > noSection ? 1 : index + 1)">></a>
        </section>
...
export default {
  ...
  methods: {    
    getCategorieFavorite: async function () {
      this.$axios
  ...
  async created() {
    await Promise.all([this.getCategories(),this.getCategorieFavorite()]);
    this.getMovie();
  },
};
</script>
